...
$outputPath = Join-Path $outputDir "desktop-rust-v192.json"
...
if ($Mode -in @("fmt-check", "verify")) {
  Invoke-RustCargo @("fmt", "--manifest-path", $manifest, "--", "--check")
  $checks.fmt = "passed"
}
if ($Mode -in @("check", "verify")) {
  Invoke-RustCargo @("check", "--manifest-path", $manifest, "-j", "2")
  $checks.check = "passed"
}
if ($Mode -in @("test", "verify")) {
  Invoke-RustCargo @("test", "--manifest-path", $manifest, "-j", "2")
  $checks.test = "passed"
}
if ($Mode -in @("release-check", "verify")) {
  Invoke-RustCargo @("check", "--release", "--manifest-path", $manifest, "-j", "2")
...
$report = [ordered]@{
  version = "v192-desktop-rust-verification"
