...
for (const solution of library.solutions) {
  if (!solution.converged) fail(`unconverged committed solution ${solution.id}`);
...
console.log(
  `PASS SPICE audit: ${manifest.bodyOrder.length} bodies, ${manifest.rowCount} rows/body, ${(statSync(binaryPath).size / 1024 / 1024).toFixed(1)} MB`,
);
