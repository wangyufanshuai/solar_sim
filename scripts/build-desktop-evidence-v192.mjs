...
const stagePath = "dist/desktop-stage/v192/desktop-stage.json";
const stage = JSON.parse(await readFile(path.join(root, stagePath), "utf8"));
if (stage.version !== "v192-desktop-compact") {
  throw new Error(`Unexpected desktop stage version: ${stage.version}`);
}
...
const report = {
  version: "v192-desktop-compact-build-evidence",
  generatedAt: new Date().toISOString(),
  profile: "desktop-compact",
  stage: {
    path: stagePath,
    sha256: await sha256(stagePath),
    bytes: stage.bytes,
    files: stage.files,
    nextBuildId: stage.nextBuildId,
  },
  webviewInstallMode: "embedBootstrapper",
  artifacts,
  installQa: "pending-clean-windows-11-vm",
...
await writeFile(
  path.join(root, "dist/science/desktop-build-v192.json"),
  `${JSON.stringify(report, null, 2)}\n`,
);
console.log(`v192 desktop build evidence: ${artifacts.length} unsigned installers`);
