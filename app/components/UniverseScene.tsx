...
function PresentationCameraBridge({
  presentationMode,
  scaleMode,
  controlsRef,
}: {
  ...
  useEffect(() => {
    ...
    camera.fov = presentationMode === "orbit-atlas" ? 54 : 60;
...
function LodOrbitControlsBridge({ floatingOriginRef, controlsRef, presentationMode }: { floatingOriginRef: MutableRefObject<FloatingOriginState>; controlsRef: MutableRefObject<OrbitControlsImpl | null>; presentationMode: SolarPresentationMode }) {
  useFrame(() => {
    const controls = controlsRef.current;
    if (controls) {
...
function CameraFocusBodyBridge({ physicsRef, floatingOriginRef, earthMoonView, cameraBodyFocusRequest, controlsRef, presentationMode, atlasScaleMode }: { physicsRef: MutableRefObject<SolarSystemPhysicsRef | null>; floatingOriginRef: MutableRefObject<FloatingOriginState>; earthMoonView: boolean; cameraBodyFocusRequest?: CameraBodyFocusRequest | null; controlsRef: MutableRefObject<OrbitControlsImpl | null>; presentationMode: SolarPresentationMode; atlasScaleMode: OrbitAtlasScaleMode }) {
  const camera = useThree((s) => s.camera);
  ...
  }, [atlasScaleMode, camera, controlsRef, floatingOriginRef, physicsRef, presentationMode]);
  ...
  return (
    <RelativisticOpticsProvider>
      <BloomSceneProvider>
        ...
        <BrightStarTierBridge floatingOriginRef={simulation.floatingOriginRef}>{(tier2) => <ScienceBackdrop floatingOriginRef={simulation.floatingOriginRef} brightStarTier2={tier2} presentationMode={simulation.presentationMode} onSkyReady={simulation.onSkyReady} />}</BrightStarTierBridge>
        {!orbitAtlas ? (
        ...
        <CameraFocusBodyBridge physicsRef={simulation.physicsRef} floatingOriginRef={simulation.floatingOriginRef} earthMoonView={simulation.earthMoonView} cameraBodyFocusRequest={simulation.cameraBodyFocusRequest} controlsRef={controlsRef} presentationMode={simulation.presentationMode} atlasScaleMode={simulation.atlasScaleMode} />
        <CameraFocusDirectionBridge controlsRef={controlsRef} />
        {simulation.viewSettings.showReferenceOrbits ? <ReferenceOrbitDecor presentationMode={simulation.presentationMode} scaleMode={simulation.atlasScaleMode} renderBudget={simulation.atlasRenderBudget} /> : null}
        ...
        {simulation.localLaunchActive && simulation.onLocalLaunchHandoff ? (
          ...
          <>
            <LabelOcclusionProvider>
              <SolarSystemBodies physicsRef={simulation.physicsRef} floatingOriginRef={simulation.floatingOriginRef} onSelectBody={simulation.onSelectBody} onBodyCanvasPick={simulation.onBodyCanvasPick} selectedBodyIndex={simulation.selectedBodyIndex} earthMoonView={simulation.earthMoonView} viewSettings={simulation.viewSettings} simDaysRef={simulation.simDaysRef} presentationMode={simulation.presentationMode} atlasScaleMode={simulation.atlasScaleMode} onReady={simulation.onCoreBodiesReady} />
