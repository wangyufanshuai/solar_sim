...
const ATLAS_BODY_RADIUS: Readonly<Record<string, number>> = {
  sun: 6.4,
  mercury: 1.15,
  venus: 1.55,
  earth: 3.0,
...
function SaturnRings({
  ...
  const opacityMul = orbitAtlas ? 0.72 : 1;
  const mainRingColor = orbitAtlas ? "#b7aa8e" : "#d4c4a4";
  return (
    <group>
...
function BodyShell({
  ...
  const visualRadius = atlasCompressed
    ? ATLAS_BODY_RADIUS[def.id] ?? Math.max(0.28, Math.min(0.72, def.radiusScene * 2.4))
  ...
  return (
    <group ref={(g) => { (groupRef as React.MutableRefObject<THREE.Group | null>).current = g; bodyRefs.group = g; }} frustumCulled={false}>
      ...
      <group ref={visualRef} frustumCulled={false}>
        {def.showRings ? <SaturnRings radiusScene={visualRadius} orbitAtlas={atlasCompressed} /> : null}
        <CelestialBody
          variant="planet"
          bodyId={def.id}
          radius={visualRadius}
          ...
          spinAngleRef={spinAngleRef}
          atlasVisualProfile={atlasBodyProfile}
          forceSurface={atlasCompressed && isMajor}
        />
      </group>
    </group>
  );
