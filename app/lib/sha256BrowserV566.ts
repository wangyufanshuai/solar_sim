...
export function sha256BytesHexV566(source: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h! + sum1 + choose + ROUND[index]! + schedule[index]!) >>> 0;
...
export function sha256Utf8HexV566(value: string): string {
  return sha256BytesHexV566(new TextEncoder().encode(value));
}
