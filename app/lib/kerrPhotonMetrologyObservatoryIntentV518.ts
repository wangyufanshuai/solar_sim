import type { AtlasDeliveryProfile } from "./atlasDeliveryProfile";
import type { KerrPhotonMetrologyDetailSurfaceV517 } from "./kerrPhotonMetrologyObservatoryIntentV517";

export const KERR_PHOTON_METROLOGY_OBSERVATORY_INTENT_VERSION_V518 =
  "v518-kerr-photon-metrology-observatory-intent-v1" as const;
export type KerrPhotonMetrologyDetailSurfaceV518 =
  | KerrPhotonMetrologyDetailSurfaceV517
  | "provenance-constellation";

export function resolveKerrPhotonMetrologyObservatoryIntentV518(
  deliveryProfile: AtlasDeliveryProfile,
  campaignEvidenceActive: boolean,
) {
  const authorized = deliveryProfile === "local-shadow" && campaignEvidenceActive;
  return Object.freeze({
    version: KERR_PHOTON_METROLOGY_OBSERVATORY_INTENT_VERSION_V518,
    deliveryProfile,
    campaignEvidenceActive,
    observatoryImportAuthorized: authorized,
    automaticComponentImportBudget: authorized ? 1 : (0 as 0 | 1),
    detailComponentCatalogSize: 20 as const,
    concurrentDetailSurfaceBudget: authorized ? 1 : (0 as 0 | 1),
    provenanceNodeCount: 8 as const,
    provenanceEdgeCount: 7 as const,
    visualModeCount: 2 as const,
    staticStatusNetworkRequestBudget: 0 as const,
    staticStatusScientificWriteback: false as const,
    canvasCreated: false as const,
    sceneRevisionDelta: 0 as const,
    physicsMutationAllowed: false as const,
    reason: authorized
      ? ("enabled" as const)
      : !campaignEvidenceActive
        ? ("inactive" as const)
        : deliveryProfile === "vercel-lite"
          ? ("lite-boundary" as const)
          : ("standalone-boundary" as const),
  });
}
